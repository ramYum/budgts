...
| **Vercel** | Web hosting / deployment | Next.js web app, API routes and server actions. Two projects: `budgts` (prod, `budgts.com`) and `budgts-staging` | `docs/deploy.md`. Hobby plan; upgrade deferred to the launch-readiness milestone. **The production project must move off Hobby to a paid plan before Budgts launches as a paid product** (release prerequisite, `docs/roadmap.md`). |
| **Supabase** | Database + auth backend | Postgres, Auth (Magic Link + Google identity), RLS, Storage/Realtime; separate staging and prod projects. Also hosts the `pg_cron` poller that calls `/api/plaid/sync-due` | `docs/deploy.md`, `.env.local.example`. Free/Nano tier (500MB, pauses after 7 idle days); upgrade deferred. The cron job is dashboard-configured, not in `supabase/migrations/`. |
| **Plaid** | Bank connectivity | Bank linking (Link + OAuth institutions), accounts, transaction sync, webhooks (`/api/plaid/webhook`, signature-verified), `/item/remove` on disconnect | `src/app/api/plaid/*`, `src/lib/plaid/*`. Production live on prod; Sandbox on staging. Per-item cost applies. |
...
| **Google** | Authentication provider | "Sign in with Google" (OAuth) on **web and mobile**; Google Cloud OAuth consent-screen branding | `src/app/(auth)/sign-in/sign-in-form.tsx`, `src/server/auth.ts`. Consent screen is still in *Testing* until privacy/terms links are added. |
...
| **RevenueCat** | Subscription / entitlement layer | Normalizes App Store + Play Billing events. Webhook (`/api/billing/webhook/revenuecat`, HMAC-signature verified) → entitlement + ledger; server-side reconciliation via its REST API; the mobile app uses its SDK (`react-native-purchases`, public key only). Supabase user id = RevenueCat `app_user_id` | Adapter + SDK integration built and tested with fixtures (`src/lib/billing/revenuecat/*`, `mobile/lib/billing/*`; design: `docs/specs/2026-09-21-v1-monetization-design.md`). **Not live:** no RevenueCat project, keys, or store products exist yet. Separate staging/production projects and keys required. |
...
- **Auth email delivery.** No custom SMTP is documented; Magic Link mail presumably rides Supabase's built-in sender, which is rate-limited and not meant for production volume. Not verifiable from the repo — check the Supabase dashboard.
- **Error monitoring / analytics.** No crash reporting, product analytics or uptime monitoring appears in the code or docs.
- **Resend (transactional email) — trial-end reminder.** Owner-chosen channel. Adapter built and unit-tested behind the provider-neutral `ReminderDelivery` port (`src/lib/billing/email/resend.ts`; `POST https://api.resend.com/emails`, `Idempotency-Key` per user+trial end, staging recipient allowlist so staging never emails a real customer). **Not live:** no Resend account/key, and no verified sending domain exists. Requires a domain Budgts owns verified in Resend (SPF/DKIM records; Resend shows the exact records) — **DNS changes need owner approval and have not been made.** Plan facts: Free = 3,000 emails/month with a 100/day cap (the daily cap is the launch-scale risk: a burst of trial endings >100/day would delay reminders); Pro = $20/month. No paid plan purchased. Separate staging/production keys.
