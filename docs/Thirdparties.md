...
| **RevenueCat** | Subscription / entitlement layer | Normalizes App Store + Play Billing events. Webhook (`/api/billing/webhook/revenuecat`, HMAC-signature verified) → entitlement + ledger; server-side reconciliation via its REST API; the mobile app uses its SDK (`react-native-purchases`, public key only). Supabase user id = RevenueCat `app_user_id` | Adapter + SDK integration built and tested with fixtures (`src/lib/billing/revenuecat/*`, `mobile/lib/billing/*`; design: `docs/specs/2026-09-21-v1-monetization-design.md`). **Project created** (owner, 2026-09-22; project "Budgts"). Store products/keys/webhook wiring still blocked on Google Play developer identity verification (see Planned). **Plan:** webhooks need the Pro plan (free until $2,500 monthly tracked revenue, then 1%); nothing has been purchased. |
...
| **Google Play Console** | Android distribution | Enrollment ($25 one-time), store listing, Google Play Billing | **Personal developer account created** (owner, 2026-09-22). Google identity verification pending — blocks app entry, subscription products and RevenueCat's Play integration until approved. |
