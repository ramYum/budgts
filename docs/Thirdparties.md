...
- **Trial-end reminder channel (email / push).** Budgts sends its own reminder ~24 h before a trial converts, but no notification vendor exists (nothing in web or mobile). The scheduling/claim domain is built provider-neutral behind a `ReminderDelivery` port; **choosing a channel (e.g. Expo Push, Resend, Postmark) is an open owner decision** and must be added here when made (`docs/specs/2026-09-21-v1-monetization-design.md` §7).
