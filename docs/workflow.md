...
| — | **First-run tour (v1) — ⛔ REMOVED from the release path 2026-09-20 (`7468365`)** | The deferred 3-screen onboarding wizard, rebuilt as a convenience-first pitch (auto-capture + auto-categorization) picked up once Plaid went live. New `profiles.tour_seen_at` (migration `0014`), pure `buildTourSteps` (TDD), shared `TourCard`/`TourWizard`, `/onboarding` restructured (Welcome → Every purchase, tracked → Currency) feeding into new `/tour` (Connect your bank → Sorted for you → Know what's left → All set), dashboard gate, Help replay link (now also links to the new How Budgts Works guide — see below). Domain + component tests green, e2e specs updated to a shared `onboardAndSkipTour` helper + new `tour.spec.ts`. `lint`/`typecheck`/`test`/`build` all green on branch `v1.5/first-run-tour`. Spec: `docs/specs/2026-09-15-first-run-tour-design.md`. **Migration `0014` not yet applied to production** — blocked by the auto-mode production-deploy guard, needs the owner to run `npm run db:migrate` before this deploys (see gate's fail-safe in the spec). E2E not run in this session: `.env.local` points at the live production Supabase project with no separate dev/staging DB, so an e2e run here would create/delete real rows against prod. A **v2 redesign (live coachmarks, spotlight + tooltip on the real button on the real page)** has since been fully specced and planned — `docs/specs/2026-09-15-first-run-tour-live-coachmarks-design.md`, `docs/superpowers/plans/2026-09-15-first-run-tour-coachmarks.md` — **but that plan's tasks have not been executed**; this v1 tour is still what actually runs today. | ⛔ removed on `mobile/native-home` (history) | `e96a428` → removed `7468365` |
| — | **"How Budgts Works" guide — ⛔ REMOVED from the release path 2026-09-20 (`7468365`)** | Permanent static Help page (`/help/how-it-works`) teaching the end-to-end workflow: connect → transactions arrive automatically → auto-categorize → review exceptions → set a budget → Money Left → track progress. Core message: "You spend. Budgts keeps track." Linked from a new entry card atop `/help` and from the v1 tour's final card. No DB reads, no new dependency; 3 new `NavIcon` glyphs (`categorize`/`review`/`money-left`) added to the existing shared icon set. Component tests for the guide page, the updated Help page, and the tour's new link. `lint`/`typecheck`/`test`/`build` all green. Spec: `docs/specs/2026-09-15-how-budgts-works-guide-design.md`. | ⛔ removed on `mobile/native-home` (history) | removed `7468365` |
| — | **Native Home + mobile auth** (`mobile/native-home`, PR #1 draft) | First real native Home over `GET /api/mobile/home` (Bearer, RLS-scoped); mobile magic-link / Google return to `budgts://auth/callback`; Hermes currency formatting verified on a device. | ✅ done (branch; not on `main`) | `9cae8db` |
| — | **Account deletion** (App Store / Play requirement) | `deleteAccount`: Path A hard delete (no ledger history) / Path B anonymize keeping the immutable ledger; fails closed; strict Plaid Item removal; database-side write guard (0019/0020); FK indexes (0017/0018) after a 25k-row user timed out both paths; Auth-delete deadlock retry; Path B atomic + plan-cache fix; step-up re-auth; Bearer-token support. Real-Postgres + real-Auth + Plaid-sandbox coverage. Spec: `docs/specs/2026-09-19-account-deletion-design.md`. | ✅ done (branch; migrations 0017–0020 not applied to production) | `60a8938` |
| — | **V1 monetization** | Full monetization ledger `0021` + `entitlements` / `billing_events` `0022`; provider-neutral entitlement domain (`hasPremium`, pure reducer); RevenueCat adapter (signed webhook, env fence, reconcile); idempotent atomic event processor; entitlement + refresh + cron endpoints; server-authoritative mobile purchase flow; 14-day store-managed trial; trial-end reminder (Resend adapter, atomic claim + lease); Manage Subscription (Settings, `/manage-subscription`, deletion flow); deletion consults the billing provider. Real-connection concurrency suite. Spec: `docs/specs/2026-09-21-v1-monetization-design.md`. | ✅ done + staging-verified (branch); RevenueCat / Resend / store products **not live** | `c4893d9` … `fc9c6f4` |
| — | **Staging replaced (2026-09-21)** | New single staging project `Budgets-Staging-3` (`uvowywszaiojboaxdmoz`) migrated from empty (0000→0022, ledger 23/23), Vercel `budgts-staging` (Production + Preview targets) re-pointed and redeployed, pg_cron jobs (`plaid-sync-due`, `billing-reminders`, `billing-reconcile`) scheduled, Auth (Site URL, allow-list, Google) configured by the owner. Verified: integration suite 163/163, deployed end-to-end check 18/18, Playwright 17/17. The old `Budgts-Staging-2` was then deleted by the owner. Found + fixed on the way: the drizzle-wrapped connection cannot take Date params (`fromPostgres` adapts at the port), and tests that assumed a cascade lock order / client clock. Runbook: `docs/operations/staging-replacement.md`. | ✅ done | `a77fc88` |
...
complete** (closed 2026-09-14, `b08cfcf`). V1.5 (recurring / subscription / bill /
paired-transfer detection) is live too. **As of 2026-09-21** the mobile / deletion / monetization work sits on the draft branch
`mobile/native-home` (PR #1), staging-verified but not on `main` and not in production; the next step is owner-approved release
preparation. V2 (email / receipt ingestion) and V2+ (AI assistant) are post-launch. Full ladder: `docs/roadmap.md`; working detail: §4 below.
...
Expo/React Native + Expo Router in `mobile/`; reuse domain logic + Supabase; EAS Build
(required — owner is on Windows, cannot build iOS locally). Prereqs: Apple
Developer Program ($99/yr), Google Play Console ($25 once) — not enrolled yet.
**State (2026-09-21):** native Home, mobile auth, account deletion and V1 monetization are built on `mobile/native-home` and
staging-verified — see the status board above and `docs/roadmap.md` (Native apps track) for what is still owner / release work.
...
- **2026-09-17 → 2026-09-21 — Mobile native Home, account deletion, monetization, staging replacement** (branch `mobile/native-home`, PR #1 draft;
  nothing pushed to `main` or deployed to production). See the four status-board rows above for scope and commits. Decisions worth keeping:
  the **first-run tour and "How Budgts Works" guide were removed from the release path** (`7468365`, 2026-09-20; a replacement is separate
  work); the trial is **14 days, store-managed, auto-converting** (supersedes the 7 days in the earlier specs); **RevenueCat stays at the provider
  boundary**; **Plaid is never a payment method** and there is **no web billing in V1**; the ledger keeps all ten tables from `0021` even though
  several are inert (no invented functionality); a paid-history user is never hard-deleted — even when the billing provider, not the local
  ledger, is what reveals the charge. `0021`/`0022` renumbered after `0020` so the chain has no gap. A **single staging project** replaced
  two earlier ones after a ledger drift (`docs/operations/database-migrations.md`).
