...
Regenerates every app icon (manifest icons, maskable icons, the iOS
apple-icon and the browser-tab icon) from the one pixel-robin source,
`src/lib/brand/robin-art.ts`. Integer px-per-cell scales keep the pixel art
crisp; maskable variants keep the bird inside the 80% safe zone.
